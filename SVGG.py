...
class SCNN(nn.Module):  #VGG9
    def __init__(self, timesteps=time_steps, leak_mem=0.95,
                 img_size=32 if dataset_name == 'cifar10' else 28,  num_cls=10):
        ...
        self.conv1 = nn.Conv2d(3 if dataset_name == 'cifar10' else 1, 64, kernel_size=3, stride=1, padding=1, bias=bias_flag)
    ...
    def forward(self, inp, temp=1):
        ...
        output_spike = torch.zeros(self.timesteps, batch_size, self.num_cls).cuda()
        ...
        for t in range(self.timesteps):
            ...
            output_spike[t] = self.fc2(out_prev)

        out_voltage = mem_fc2 / self.timesteps

        if frame == 'fedDis' or frame == 'fedAd1':
            return out_voltage, output_spike
        else:
            return out_voltage

    def feature_forward(self, inp, temp=1):
        batch_size = inp.size(0)
        mem_conv1 = torch.zeros(batch_size, 64, self.img_size, self.img_size).cuda()
        mem_conv2 = torch.zeros(batch_size, 64, self.img_size, self.img_size).cuda()
        mem_conv3 = torch.zeros(batch_size, 128, self.img_size // 2, self.img_size // 2).cuda()
        ...
        output_spike = torch.zeros(PoissonGen(inp).size())
        for t in range(self.timesteps):
            ...
            if t == 0:
                output_spike = out_prev
            else:
                output_spike += out_prev

        output_spike = output_spike / self.timesteps
        return output_spike, output_spike
